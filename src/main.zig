...
pub fn promptCommand(prompt_str: []const u8, context: anytype, cmds: []const CommandDescriptor) !bool {
    ...
    const words = parseUserInput: {
        ...
        while (i < user_input.len) : (i += 1) {
            ...
            const word_start = i;
            var word_end = i;
            while (i < user_input.len) : ({i += 1; word_end += 1;}) {
                if (std.ascii.isSpace(user_input[i])) {
                    break;
                }
            }
...
fn parse(comptime T: type, string: []const u8) !T {
    ...
    return switch (T) {
        []const u8 => string,
            ...
            .Bool => if (std.mem.eql(u8, string_lowered, 'y')) true
                else if (std.mem.eql(u8, string_lowered, 'n')) false
                else error.FailedToParseBool,
